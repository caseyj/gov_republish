defmodule AtProto.IdentityResolutionTest do
  ...
  for {did_doc, handle, expected} <- [
    ...
    test "Show handle_in_doc verifies handles correctly #{did_doc} and handle #{handle}" do
      assert IdentityResolution.handle_in_doc(
               %{"alsoKnownAs" => unquote(did_doc)},
               unquote(handle)
             ) == unquote(expected)
    end
  end
  ...
  for {did_doc, handle, expected} <- [
    ...
    test "Show did_equals_doc_id verifies handles correctly #{did_doc} and handle #{handle}" do
      assert IdentityResolution.did_equals_doc_id(%{"id" => unquote(did_doc)}, unquote(handle)) ==
               unquote(expected)
    end
  end
        ...
        get: fn _a ->
          if unquote(status) do
            if unquote(is_garbage) do
              ...
              {:ok,
               %HTTPoison.Response{status_code: unquote(status), body: "{\"hello\":\"world\"}"}}
            end
          else
            {:error, %HTTPoison.Error{}}
          end
